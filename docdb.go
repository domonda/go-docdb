...
func AddDocumentVersion(ctx context.Context, docID, userID uu.ID, reason string, txFunc AddVersionTx) (versionInfo *VersionInfo, err error) {
	defer errs.WrapWithFuncParams(&err, ctx, docID, userID, reason)

	return conn.AddDocumentVersion(ctx, docID, userID, reason, txFunc)
}

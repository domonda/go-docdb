...
- `localfsdb.Conn.ReadDocumentVersionFile` reads through `DirFileProvider` too, instead of opening the file in the version directory directly. It returned the content of a hidden file that `DocumentVersionFileProvider` denied the existence of, so two `Conn` methods disagreed about which filenames belong to a version.
- `localfsdb.Conn.CreateDocument` rejects a document whose files are all hidden instead of writing a version with no files at all. It validated the passed files, but the version's file list is derived from what a reader of the version will get, and hidden files are not part of that — so the resulting genesis version could be empty, which no version may be.
...
- The package documentation states the hidden-file policy the filtering above implements: a hidden file — a dot-prefixed name everywhere, plus the hidden attribute on Windows — is never a file of a document version, neither read as one nor saved as one, and `Conn.CreateDocument` and `CreateVersionResult` say what that means for files handed in. It rests on the assumption that no already stored version tracks a hidden file: nothing enforced that before, so a version whose metadata does list one now fails to read, its file being tracked but no longer provided.

...
### Added
- `storeconn.DocumentStore.DocumentHashFilesExist(ctx, docID, files []docdb.FileInfo) ([]bool, error)`: reports for every passed file whether the store holds a file with that name and content hash, one result per file in the same order. A `DocumentStore` deliberately does not track versions — files are addressed by name and content hash — so this is how a caller determines whether a complete version is present: it is, exactly when all of its files are. The batch shape is what keeps the merge-restore check to one round trip for a whole document; `s3store` answers it from a single prefix listing rather than a `HeadObject` per file.
- `docdb.VersionInfo.SetFileDeltas(prevFiles map[string]FileInfo)`: derives `AddedFiles`, `ModifiedFiles`, and `RemovedFiles` by comparing `Files` against the complete file set of the previous version (pass `nil` for the first version of a document, which reports every file as added). The same diff was hand-written in three places — `HashedDocument.VersionInfo`, `localfsdb`'s `newVersionInfo`, and `storeconn`'s merge-restore — and their results are compared against each other: a `MetadataStore` in versions-exist mode verifies the `VersionInfo` a restore would insert against the stored one, so a divergence between two copies of the derivation does not fail a test at build time but a migration halfway through a company's documents. All three now call `SetFileDeltas`. It compares only filenames and content hashes, so a caller that knows just those of the previous version can build `prevFiles` without looking up file sizes; the lists are replaced rather than appended to, sorted by filename so they never depend on map iteration order, and left nil instead of empty.
